/**
  * Copyright(C), 2018
  * Name: intersection_of_quard_tree
  * Author: Wilson Lan
  * Description:
  ...
  *
  *     We want to store True/False information in our quad tree. The quad
  *     tree is used to represent a N * N boolean grid. For each node, it
  *     will be subdivided into four children nodes until the values in the
  *     region it represents are all the same. Each node has another two
  *     boolean attributes : isLeaf and val. isLeaf is true if and only if
  *     the node is a leaf node. The val attribute for a leaf node contains
